<img width="2557" height="1439" alt="image" src="https://github.com/user-attachments/assets/0b9f5d24-6bd4-4f62-b58c-950c209fd1d3" />

<img width="2559" height="1439" alt="image" src="https://github.com/user-attachments/assets/24044c0a-20c0-4ed2-84a6-90616e696092" />

<img width="2518" height="1383" alt="image" src="https://github.com/user-attachments/assets/de7b9db1-2e10-46e8-ae35-a8e0656b28f9" />
...
-- ============================================================
-- 00A - Tracked Keyword Logic Reference
-- ============================================================

CREATE OR REPLACE TABLE BI.NEWS.PHARMA_NEWS_PRIORITY_KEYWORDS AS
SELECT
...
-- ============================================================
-- 00B - Tracked Companies
-- ============================================================

CREATE OR REPLACE TABLE BI.NEWS.PHARMA_NEWS_TRACKED_COMPANIES AS
SELECT
...
-- ============================================================
-- 05 - Priority Tier V2
-- Uses tracked companies, keywords, value thresholds, policy/location/modality rules
-- ============================================================

CREATE OR REPLACE VIEW BI.NEWS.V_PHARMA_NEWS_PRIORITY_TIER_V2 AS
WITH BASE AS (
    SELECT
        C.*,
        LOWER(
    ...
    FROM BI.NEWS.PHARMA_NEWS_CLASSIFIED C
...
COMPANY_MATCHES AS (
    ...
    FROM BASE B
    LEFT JOIN BI.NEWS.PHARMA_NEWS_TRACKED_COMPANIES TC
...
CREATE OR REPLACE VIEW BI.NEWS.V_PHARMA_NEWS_DIGEST_QUEUE_V2 AS
WITH CANDIDATES AS (
    SELECT
        RECEIVED_TS_PARSED,
        EMAIL_SOURCE_TYPE,
    ...
    FROM BI.NEWS.V_PHARMA_NEWS_PRIORITY_TIER_V2
...
-- Currently using verified DTU email
-- ============================================================
...
CREATE OR REPLACE PROCEDURE BI.NEWS.SP_SEND_DAILY_PHARMA_NEWS_DIGEST_V2(P_DAYS_BACK NUMBER)
RETURNS STRING
LANGUAGE SQL
EXECUTE AS CALLER
AS
...
-- It uses 1 day for normal daily production.
-- Task is created suspended by default.
-- ============================================================

CREATE OR REPLACE TASK BI.NEWS.TASK_SEND_DAILY_PHARMA_NEWS_DIGEST_V2
    WAREHOUSE = COMPUTE_WH
    SCHEDULE = 'USING CRON 15 8 * * MON-FRI Europe/Copenhagen'
AS
    CALL BI.NEWS.SP_SEND_DAILY_PHARMA_NEWS_DIGEST_V2(1);


-- ============================================================
-- 10 - Test the V2 digest
-- This sends one email using the last 7 days.
-- Change 7 to 1 for daily production testing.
-- ============================================================
...
-- Activate daily task when ready:
-- ALTER TASK BI.NEWS.TASK_SEND_DAILY_PHARMA_NEWS_DIGEST_V2 RESUME;

-- Pause daily task:
-- ALTER TASK BI.NEWS.TASK_SEND_DAILY_PHARMA_NEWS_DIGEST_V2 SUSPEND;

-- Execute task manually:
-- EXECUTE TASK BI.NEWS.TASK_SEND_DAILY_PHARMA_NEWS_DIGEST_V2;
