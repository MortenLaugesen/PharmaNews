-- ============================================================
-- PHARMA NEWS MONITORING - CLEAN V1.4
...
-- 2. Clean article view with publish date
-- 3. Subject gate
-- 4. AI relevance
-- 5. AI classification
-- 6. Priority scoring
-- 7. Final digest queue
-- 8. AI summaries, now 1-3 paragraphs
-- 9. AI semantic deduplication / story keys
-- 10. Professional HTML email view with publish date
...
-- - Added publish date
-- - Expanded summary from one short sentence to 1-3 paragraphs
-- - Summary now focuses on monetary value, capability expansion/reduction,
--   and expected industry impact
-- ============================================================
...
ITEMS AS (
    SELECT
        *,
        ...
        COALESCE(TO_VARCHAR(PUBLISH_DATE, 'YYYY-MM-DD'), 'Unknown') AS PUBLISH_DATE_HTML,
        REPLACE(REPLACE(REPLACE(COALESCE(PUBLISH_DATE_SOURCE, ''), '&', '&amp;'), '<', '&lt;'), '>', '&gt;') AS PUBLISH_DATE_SOURCE_HTML,
            ...
            '<tr>
                <td style="padding:16px 0; border-bottom:1px solid #e5e5e5;">
                    ...
                    <div style="font-size:13px; color:#666666; margin-bottom:8px;">
                        <b>Publish date:</b> ' || PUBLISH_DATE_HTML || '
                        <span style="color:#999999;">(' || PUBLISH_DATE_SOURCE_HTML || ')</span>
                    </div>' ||
...
WHERE SUMMARY_VERSION = 'v2_1_3_paragraphs'
