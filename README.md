...
SELECT COUNT(*) AS PRIORITIZED_ROWS
...
SELECT
    ...
    COUNT_IF(
        ...
        OR TRIM(ARTICLE_TITLE) = ''
    ) AS MISSING_ARTICLE_TITLES,
    COUNT_IF(
        BODY_BEST IS NULL
        OR TRIM(BODY_BEST) = ''
    ) AS MISSING_BODY_TEXT,
    COUNT_IF(
        ARTICLE_URL IS NULL
        OR TRIM(ARTICLE_URL) = ''
    ) AS MISSING_ARTICLE_URLS,
    MIN(RECEIVED_TS) AS OLDEST_EMAIL,
    MAX(RECEIVED_TS) AS NEWEST_EMAIL
...
SELECT
    ...
    ARTICLE_TITLE,
    ARTICLE_URL,
    LEFT(BODY_BEST, 300) AS BODY_PREVIEW
FROM PHARMA_NEWS_SANDBOX.NEWS.STG_PHARMA_NEWS_ARTICLES_V1
ORDER BY RECEIVED_TS DESC
LIMIT 20;
